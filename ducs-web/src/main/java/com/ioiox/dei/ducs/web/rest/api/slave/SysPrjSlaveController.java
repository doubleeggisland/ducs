...
public class SysPrjSlaveController {

    @Autowired
    @Qualifier("sysPrjSlaveStdDataSvc")
    private SysPrjSlaveStdDataSvc sysPrjSlaveStdDataSvc;

    @PostMapping(path = "/sys-prjs/count", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public DeiResponseData<Integer> countSysPrjs(@RequestBody final SysPrjQueryParam queryParam) {
        return new DeiResponseData.Builder<Integer>()
                .code(DeiResponseData.SUCCESS)
                .success(DeiGlobalConstant.TRUE_STR)
                .data(sysPrjSlaveStdDataSvc.countByParam(queryParam))
                .build();
    }
